...
Target::Target(GLuint tex, int score, float r, float px, float py, float pz, float rx, float ry, float rz)
	: TexturedPlane(tex, px, py, pz,rx, ry, rz), m_score(score), m_originalTexture(tex), m_hit(false)
{
	transform.scale = { r * 2,r * 2,r * 2 };
}
